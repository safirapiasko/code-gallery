...
namespace NS_TRBDF2 {
  ...
  assemble_rhs_cell_term_velocity(const MatrixFree<dim, Number>&               data,
    ...
    if(TR_BDF2_stage == 1) {
      ...
      // FEEvaluation<dim, fe_degree_p, n_q_points_1d_v, 1, Number>   phi_visc(data, 0);
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        // phi_visc.reinit(cell);
        // phi_visc.gather_evaluate(src[3], EvaluationFlags::nothing);
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          // const auto& visc_n = phi_visc.get_value(q);

          phi.submit_value(1.0/(gamma*dt)*u_n, q); /*--- 'submit_value' contains quantites that we want to test against the
                                                          test function ---*/
          // phi.submit_gradient(-a21/Re*visc_n*grad_u_n + a21*tensor_product_u_n + p_n_times_identity, q);
  ...
  protected:
    ...
    EquationData::Viscosity<dim> visc_init;
    ...
    FESystem<dim> fe_viscosity;

    /*--- Handler for dofs ---*/
    DoFHandler<dim> dof_handler_velocity;
    DoFHandler<dim> dof_handler_pressure;
    DoFHandler<dim> dof_handler_viscosity;

    /*--- Quadrature formulas for velocity and pressure, respectively ---*/
    QGauss<dim> quadrature_pressure;
    QGauss<dim> quadrature_velocity;
    QGauss<dim> quadrature_viscosity;

    /*--- Viscosity vector ---*/
    LinearAlgebra::distributed::Vector<double> visc_n;
  ...
  NavierStokesProjection<dim>::NavierStokesProjection(RunTimeParameters::Data_Storage& data):
    ...
    fe_viscosity(FE_DGQ<dim>(EquationData::degree_p), 1),
    dof_handler_velocity(triangulation),
    dof_handler_pressure(triangulation),
    dof_handler_viscosity(triangulation),
    quadrature_pressure(EquationData::degree_p + 1),
    quadrature_velocity(EquationData::degree_p + 2),
    quadrature_viscosity(EquationData::degree_p + 1),
  ...
  void NavierStokesProjection<dim>::setup_dofs() {
    ...
    dof_handler_viscosity.distribute_dofs(fe_viscosity);
    ...
    if(Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0) {
      ...
      output_n_dofs_viscosity << dof_handler_viscosity.n_dofs() << std::endl;
    }
    ...
    dof_handlers.push_back(&dof_handler_viscosity);
    ...
    constraints_viscosity.clear();
    constraints_viscosity.close();
    ...
    constraints.push_back(&constraints_viscosity);
    ...
    matrix_free_storage->initialize_dof_vector(visc_n, 0);
  ...
  void NavierStokesProjection<dim>::initialize() {
    ...
    VectorTools::interpolate(dof_handler_pressure, visc_init, visc_n);
  ...
  void NavierStokesProjection<dim>::diffusion_step() {
    ...
    if(TR_BDF2_stage == 1) {
      navier_stokes_matrix.vmult_rhs_velocity(rhs_u, {u_n, u_extr, pres_n, visc_n});
      navier_stokes_matrix.set_u_extr(u_extr);
      u_star = u_extr;
    }
    else {
      navier_stokes_matrix.vmult_rhs_velocity(rhs_u, {u_n, u_n_gamma, pres_int, u_extr, visc_n});
      navier_stokes_matrix.set_u_extr(u_extr);
      u_star = u_extr;
    }
