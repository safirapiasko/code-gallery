    ...
    if(TR_BDF2_stage == 1) {
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto lambda_old             = (boundary_id == 1) ? 0.0 : std::abs(scalar_product(phi_old.get_value(q), n_plus));
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++ face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto lambda_old             = (boundary_id == 1) ? 0.0 : std::abs(scalar_product(phi_old.get_value(q), n_plus));
          const auto lambda_int             = (boundary_id == 1) ? 0.0 : std::abs(scalar_product(phi_int.get_value(q), n_plus));
