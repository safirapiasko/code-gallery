...
namespace NS_TRBDF2 {
  ...
  assemble_rhs_cell_term_velocity(const MatrixFree<dim, Number>&               data,
    ...
    if(TR_BDF2_stage == 1) {
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        phi_force.gather_evaluate(src[3], EvaluationFlags::values);
        phi.reinit(cell);
  ...
  void NavierStokesProjection<dim>::diffusion_step() {
    ...
    if(TR_BDF2_stage == 1) {
      ...
      double force = 5.0*(1.0 - VectorTools::compute_mean_value(MappingQ1<dim>(), dof_handler_velocity, quadrature_velocity, u_n, 0));
      ...
      navier_stokes_matrix.vmult_rhs_velocity(rhs_u, {u_n, u_extr, pres_n, artificial_force});
      navier_stokes_matrix.set_u_extr(u_extr);
      u_star = u_extr;
    }
    else {
      // calculate uniform artificial force
      double force = 5.0*(1.0 - VectorTools::compute_mean_value(MappingQ1<dim>(), dof_handler_velocity, quadrature_velocity, u_n_gamma, 0));
      // interpolate force over domain
      VectorTools::interpolate(dof_handler_velocity, Functions::ConstantFunction<dim>(Vector<double>({force, 0.0})), artificial_force);

      navier_stokes_matrix.vmult_rhs_velocity(rhs_u, {u_n, u_n_gamma, pres_int, u_extr, artificial_force});
      navier_stokes_matrix.set_u_extr(u_extr);
      u_star = u_extr;
    }
