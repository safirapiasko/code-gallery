...
namespace NS_TRBDF2 {
  ...
  protected:
    ...
    FESystem<dim> fe_yplus;

    /*--- Handler for dofs ---*/
    DoFHandler<dim> dof_handler_velocity;
    DoFHandler<dim> dof_handler_pressure;
    DoFHandler<dim> dof_handler_yplus;
  ...
  NavierStokesProjection<dim>::NavierStokesProjection(RunTimeParameters::Data_Storage& data):
    ...
    fe_yplus(FE_DGQ<dim>(0), 1),
    dof_handler_velocity(triangulation),
    dof_handler_pressure(triangulation),
    dof_handler_yplus(triangulation),
  ...
  void NavierStokesProjection<dim>::create_triangulation_with_square(const unsigned int n_refines) {
    ...
    triangulation.clear();
    serial_triangulation.clear();
    pcout << "create parallel triangulation" << std::endl;
    parallel::distributed::Triangulation<dim> tria1(MPI_COMM_WORLD),
                                              tria2(MPI_COMM_WORLD),
                                              tria3(MPI_COMM_WORLD),
                                              tria4(MPI_COMM_WORLD),
    ...
    pcout << "create boundary ids for parallel triangulation" << std::endl;
    /*--- Set boundary id ---*/
    for(const auto& face : triangulation.active_face_iterators()) {
      if(face->at_boundary()) {
        const Point<dim> center = face->center();
    ...
    pcout << "create serial triangulation" << std::endl;
    /*--- single triangulation ---*/
    ...
    /*--- Set boundary id ---*/
    pcout << "set boundary ids for serial triangulation" << std::endl;
    for(const auto& face : serial_triangulation.active_face_iterators()) {
      if(face->at_boundary()) {
        const Point<dim> center = face->center();
  ...
  void NavierStokesProjection<dim>::setup_dofs() {
    ...
    dof_handler_yplus.distribute_dofs(fe_yplus);
    ...
    dof_handlers.push_back(&dof_handler_yplus);

    constraints_velocity.clear();
    constraints_velocity.close();
    constraints_pressure.clear();
    constraints_pressure.close();
    constraints_yplus.clear();
    constraints_yplus.close();
    ...
    constraints.push_back(&constraints_yplus);
    ...
    dof_handler_yplus.distribute_mg_dofs();
    ...
    for(unsigned int level = 0; level < nlevels; ++level) {
      ...
      dof_handlers_mg.push_back(&dof_handler_yplus);
  ...
  void NavierStokesProjection<dim>::initialize_nearest_boundary_point_mapping(){
    ...
    for(const auto& cell : dof_handler_yplus.active_cell_iterators()) {
  ...
  void NavierStokesProjection<dim>::compute_y_plus(LinearAlgebra::distributed::Vector<double> & vel, double lower_boundary, double upper_boundary, Point<dim> center, double object_length) {
    ...
    for(const auto& cell : dof_handler_yplus.active_cell_iterators()) {
      if(cell->is_locally_owned()) {
        ...
        // get dof indices
        std::vector<types::global_dof_index> dof_indices(fe_yplus.dofs_per_cell);
